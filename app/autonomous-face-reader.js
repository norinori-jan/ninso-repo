 ...
 *   本ファイルの明るさ/色味/彩度/左右差/光の当たり方/起伏に対する
 *   意味づけ文(brightnessTheme等)は、`data/*.js`(PDF文献から出典付き
 *   で採録した262項目のデータベース)には基づいていません。過去
 *   セッションでこのアプリが独自に設計した、娯楽目的の解釈フレーム
 *   ワークです。「色彩心理」「伝統的な見立て」等の言葉を使っていた
 *   箇所がありましたが、実際に文献と照合された事実はないため、
 *   「本アプリ独自の解釈」という表現に統一しました。今後この種の
 *   意味づけを追加・拡張する際も、文献に基づくかのような表現は
 *   避け、独自解釈であることを明記すること。
 *
...
(function (root, factory) {
  ...
  // 2026-08版で見直し: 免責(「という解釈です」「文献上の裏付けは
  // ありません」等)は、下の`LIMITATION_NOTE`にまとめて1回だけ書く方針に
  // 変更(以前は各項目の文末に繰り返し出ており、断定的に読めない・
  // 冗長という指摘があったため)。各項目のテキスト自体は言い切る。
  function brightnessTheme(avgGray) {
    if (avgGray >= 170) return { level: '明るい', text: '画面全体が明るく、開放的で前向きなエネルギーが強い状態です。' };
    if (avgGray >= 90) return { level: '中間', text: '明暗のバランスが取れており、落ち着いた心理状態です。' };
    return { level: '暗い', text: '画面全体が暗めで、内省的・慎重な心理状態、あるいは疲労や緊張が表れています。' };
  }

  function colorTheme(avgColor) {
    var warm = avgColor.r - avgColor.b;
    if (warm > 15) return { level: '暖色寄り', text: '暖色(赤み)が強く、情熱・活力・行動的なエネルギーの象徴です。' };
    if (warm < -15) return { level: '寒色寄り', text: '寒色(青み)が強く、冷静さ・落ち着き・内省的な心理状態の象徴です。' };
    return { level: '中間色', text: '色味に大きな偏りがなく、穏やかで安定した心理状態です。' };
  }

  function saturationTheme(avgSat) {
    if (avgSat > 0.35) return { level: '鮮やか', text: '色彩が鮮やかで、感情表現が豊かで活発な状態です。' };
    if (avgSat > 0.15) return { level: '中程度', text: '色彩の鮮やかさは中程度で、感情面のバランスが取れています。' };
    return { level: '控えめ', text: '色彩が控えめ(グレイッシュ)で、感情を内に秘めている、あるいは落ち着いた状態です。' };
  }
  ...
  function lightBiasTheme(lightBias) {
    ...
    if (Math.abs(h) > 5) {
      parts.push(h > 0
        ? '光は向かって左側から強く当たっており、過去・身内との結びつきの象徴です。'
        : '光は向かって右側から強く当たっており、未来・対外的な活動への意識の象徴です。');
    }
    if (Math.abs(v) > 5) {
      parts.push(v > 0
        ? '光は上側から当たっており、理想・向上心・目上からの影響を示します。'
        : '光は下側(あるいは影)が強く、地に足の着いた現実志向、または抑圧された感情を示します。');
    }
    if (!parts.length) parts.push('光の当たり方に強い偏りがなく、心理的なバランスが取れている状態です。');
    return { text: parts.join('') };
  }

  function asymmetryTheme(score) {
    if (score >= 60) return { level: '高い', text: '画像全体の左右差が比較的大きく、内面の葛藤や複数の側面(表の顔・内なる顔)が同居している可能性を示します。' };
    if (score >= 30) return { level: '中程度', text: '画像全体の左右差は中程度で、個性や表情の豊かさの表れです。' };
    return { level: '低い', text: '画像全体の左右差は小さく、安定した心理状態・調和のとれた印象です。' };
  }

  function tensionTheme(score) {
    if (score >= 60) return { level: '高い', text: '画像全体の起伏(明暗の変化)が大きく、緊張感・活発なエネルギー、あるいは強い感情の起伏を示します。' };
    if (score >= 30) return { level: '中程度', text: '適度な起伏があり、活動と休息のバランスが取れている状態です。' };
    return { level: '低い', text: '画像全体が滑らかで、リラックスした・穏やかな状態です。' };
  }

  var LIMITATION_NOTE = 'これらは画像全体の明暗・色彩・左右対称性・起伏(コントラスト)といった' +
    '大づかみな特徴から導いた、本アプリ独自の娯楽的な解釈です(data/配下の出典付き文献データとは別物で、' +
    '伝統的な相学の記述と照合されたものではありません)。' +
