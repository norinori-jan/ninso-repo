...
## ★今回(願相・検出層の精度向上)セッションで行ったこと(更新版5)
...
次回は以下を優先:
1. 【更新】精度向上策(局所適応閾値・肌色マスキング・平滑化・形状フィルタ)
   は実装済み(`app/hidden-face-detector.js`、合成データでのユニット
   テストは`npm test`で確認可能)。**次は実機(実際のブラウザ・スマホ)
   での実写真を使った自動検出の精度確認が優先**。特に肌色マスキングが
   様々な肌の色み・光源条件で偏りなく機能しているか、局所適応閾値の
   既定パラメータ(半径・thresholdK)が実写真で適切かを確認し、必要なら
   チューニングする
