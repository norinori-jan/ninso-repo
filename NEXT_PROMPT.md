...
`docs/kouza-notes/basic/KOUZA1_NOTES.md`に整理した。個別受講者への実演鑑定(実名つき)は
...
(`docs/kouza-notes/basic/KOUZA1_NOTES.md`に詳細と申し送りを記載)。コードの変更はまだ
...
`docs/kouza-notes/basic/KOUZA2_NOTES.md`参照。
...
**見送った内容(`docs/kouza-notes/basic/KOUZA2_NOTES.md`に詳細):** 裁判線の向きによる
...
(srt)を保存(動画本体は大きすぎるため含めない)。`docs/kouza-notes/basic/KOUZA3_NOTES.md`・
`docs/kouza-notes/basic/KOUZA4_NOTES.md`を追加。コード変更はなし(講座4の実演で、直前の
...
`docs/kouza-notes/basic/KOUZA5_NOTES.md`に整理。**今回はコード変更なし。** 新しく出てきた
...
## 2026-08 講座6(基本編・最終回、実演あり) + フォルダ構成の再編

基本編1〜6回目(5回目のみ2分割)の字幕をすべて読み込み終えた。
`docs/kouza-notes/basic/KOUZA6_NOTES.md`に整理。**コード変更なし。**
白目の色・白いホクロといった健康関連の話、指の状態を見る手相の話が
新出だったが、健康断定の除外方針、および「手相は顔相と別モダリティで
規模が違いすぎる」という理由でいずれも見送った。講座の最後で「画想
辞典」という資料が今後配布されるとの案内があったが未入手(次回入手
できれば`face-region-map.js`拡充に使える可能性がある)。

**基本編完走・上級編へ移行するこの区切りで、ドキュメントのフォルダ
構成を再編した(ユーザー指示):**

- `docs/KOUZAn_NOTES.md` → `docs/kouza-notes/basic/KOUZAn_NOTES.md`
- `docs/kouza-transcripts/kouzaN.srt` →
  `docs/kouza-transcripts/basic/kouzaN.srt`
- 上級編は`docs/kouza-notes/advanced/`・
  `docs/kouza-transcripts/advanced/`(`kouza-adv{n}.srt`)に置く(まだ空、
  READMEのみ配置済み)
- 各フォルダに`README.md`を配置。`GANSOU_ROADMAP.md`・本ファイル内の
  リンクもすべて新パスに更新済み。

**次回:** 上級編1〜6回目の動画はユーザー側で取得済み(圧縮版・分割版を
提供済み)。字幕が届き次第、`docs/kouza-notes/advanced/`・
`docs/kouza-transcripts/advanced/`に同様のペアで追加していく。上級編は
基本編を前提とした内容になる可能性が高いので、基本編で確立した「部位の
定義は安易に変えない/技法は都度実装してよい/健康・生死は扱わない」の
3方針を維持する。

## 2026-08 上級編(全6回)読了・天中ライン解釈の補強(最新)

上級編1〜6回目の字幕をすべて読み込み、`docs/kouza-notes/advanced/`
(`KOUZA_ADV1_NOTES.md`〜`KOUZA_ADV6_NOTES.md`+全体テーマをまとめた
`KOUZA_ADV_OVERVIEW.md`)に整理した。**ユーザー指示により、今回は基本編
のときより広く・排除せずに内容を拾った。**

**コードに反映:** `app/app.js`の天中ライン解釈で、色がはっきりしない
場合のフォールバック文言に「天中に線が達していること自体が伝統的に
大きな意味を持つ(上級4回目で明確に確認: 『天中場所にどっからでもいい
ですけど線が入ると...天中必ず登る。これ裁判』)」という一文を追加。

**実装を保留(次回ユーザーに確認したい):** 恋愛宮・夫婦宮の男女による
左右差。基本編2・上級2・上級3と**3回にわたって確認**され、
`docs/kouza-notes/README.md`の方針6(複数回確認されたものは実装を前向きに
検討する)の基準を満たしたが、正確な幾何学的対応(恋愛宮と夫婦宮が同じ
側で内外に分かれるのか、男女で左右が入れ替わるのか)が字幕だけでは
確定できなかった。実装するかどうか、次回ユーザーに確認して着手したい。

**実装しなかった内容:** 気色理論(固有色/気色/過去色。1枚の静止画では
時間変化を判定できないという技術的な理由もある)、先祖の命日・法事
(白輪、宗教的実践の助言に相当)、険難(事故・怪我・強盗被害)の相
(被害の予言に相当)。理由の詳細は
`docs/kouza-notes/advanced/KOUZA_ADV_OVERVIEW.md`参照。

`npm test`全42件に影響なし。これで基本編・上級編とも全12回分の字幕
読み込みが完了した(ユーザーコメント: 「これが動画としてのリアルな
保存」。次はユーザーが実機でアプリの精度を確認する予定)。
