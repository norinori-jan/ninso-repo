...
(function (root, factory) {
  ...
  function colorTheme(avgColor) {
    ...
    if (warm > 15) return { level: '暖色寄り', text: '暖色(赤み)が強く、情熱・活力・行動的なエネルギーを象徴する、という本アプリ独自の色彩心理的な見立てです(文献上の裏付けはありません)。' };
    if (warm < -15) return { level: '寒色寄り', text: '寒色(青み)が強く、冷静さ・落ち着き・内省的な心理状態を象徴する、という本アプリ独自の色彩心理的な見立てです(文献上の裏付けはありません)。' };
  ...
  function lightBiasTheme(lightBias) {
    ...
    if (Math.abs(h) > 5) {
      parts.push(h > 0
        ? '光は向かって左側から強く当たっており、過去・身内との結びつきを象徴する、という本アプリ独自の見立てです(文献上の裏付けはありません)。'
        : '光は向かって右側から強く当たっており、未来・対外的な活動への意識を象徴する、という本アプリ独自の見立てです(文献上の裏付けはありません)。');
    }
    if (Math.abs(v) > 5) {
      parts.push(v > 0
