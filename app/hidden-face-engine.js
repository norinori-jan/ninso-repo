...
(function (root, factory) {
  ...
  var POSITION_MEANINGS = {
    '右目の下': '本アプリ独自の解釈枠組みで、感情面・直近の心配事に関わるテーマとして割り当てた位置です(文献上の裏付けはありません)。',
    '左目の下': '本アプリ独自の解釈枠組みで、身内との関係に関わるテーマとして割り当てた位置です(文献上の裏付けはありません)。',
    '額': '本アプリ独自の解釈枠組みで、思考・プレッシャー・目上との関係に関わるテーマとして割り当てた位置です(文献上の裏付けはありません)。',
    'ほほ': '本アプリ独自の解釈枠組みで、対人関係・社会的な立場に関わるテーマとして割り当てた位置です(文献上の裏付けはありません)。',
    '口の横': '本アプリ独自の解釈枠組みで、言葉・発言・本音に関わるテーマとして割り当てた位置です(文献上の裏付けはありません)。',
    '顎': '本アプリ独自の解釈枠組みで、晩年・家庭・生活基盤に関わるテーマとして割り当てた位置です(文献上の裏付けはありません)。',
  };
  ...
  function buildGansouReport(input) {
    ...
    var positionMeaning = POSITION_MEANINGS[position] || 'この位置に対応する意味づけ(本アプリ独自の解釈枠組み)は登録されていません。参考程度に留めてください。';
    ...
    var message = '「' + position + '」に検出された願相は、向き(' + orientation + ')から見て' +
      ...
      'を示す、という本アプリ独自の解釈枠組みです。' + typeMeaning + complexityNote +
  ...
  function buildAbstractGansouReport(input) {
    ...
    if (resolvedPalaceName && PALACE_MEANINGS[resolvedPalaceName]) {
      ...
      palaceMeaning = 'この位置に対応する宮の分類は特定できませんでした。参考程度に留めてください。';
    }
    ...
    var message = '「' + locationLabel + '」' + (resolvedPalaceName ? '(' + resolvedPalaceName + ')' : '') +
      'に検出された抽象的な隠れ相は、向き(' + directionJa + ')から見て' + directionThemeText + 'を示す、という本アプリ独自の解釈枠組みです。' +
      clarityNote +
      'これは文献に基づく鑑定ではなく、本アプリが独自に設定した娯楽目的の解釈であり、実際の心理状態や人間関係を断定するものではありません。';
    if (message.length > 220) message = message.slice(0, 217) + '…';
