 ...
 *
 * ★出典についての正直な注記(2026-08時点で監査・追記):
 *   下記の POSITION_MEANINGS / ORIENTATION_MEANINGS / TYPE_MEANINGS /
 *   PALACE_MEANINGS は、`data/*.js`(PDF文献から出典付きで採録した
 *   262項目のデータベース、`source/notes.md`で採用/除外を管理)には
 *   **一切基づいていません**。過去セッションでユーザー提示の「願相検出
 *   プロンプト」というアイデアを元に、このアプリが独自に設計した
 *   解釈フレームワークです。「涙堂」「十二宮」等、伝統的な人相学の
 *   用語を借用している箇所もありますが、その意味づけの中身
 *   (例:位置ごとの心理的テーマ)は文献の記述と照合されておらず、
 *   一致している保証はありません(実際、`data/eyes.js`の
 *   `tianzhai_eyelid_area_meaning`(田宅=眉と眼の間)は「田地・家宅・
 *   目上運」を意味すると記録されているのに対し、本ファイルの
 *   `POSITION_MEANINGS['右目の下']`は「感情面・直近の心配事」としており、
 *   同じ部位でも中身が食い違う)。
 *   そのため、このファイルの意味づけを増やしたり他部位に拡張したり
 *   する場合、**文献に基づいているかのように装う表現(「伝統的な
 *   言い伝え」等)は使わないこと**。「本アプリが独自に設定した、
 *   娯楽目的の解釈枠組みであり、`data/`の文献データとは別物」と
 *   明記すること。文献に基づく意味づけを追加したい場合は、必ず
 *   対応する原典の記述を確認し、`source/notes.md`に出典を記録した上で
 *   `data/`側にデータ化するのが筋であり、このファイルに直接書き込む
 *   べきではない。
 *
...
(function (root, factory) {
  ...
  // 位置(願相が検出された領域) → 心理的意味
  //
  // ★出典なし・本アプリ独自の解釈枠組みです(上部の出典注記を参照)。
  //   `data/`配下の文献データとは対応していません。
  // -----------------------------------------------------------------
  var POSITION_MEANINGS = {
    '右目の下': '本アプリ独自の解釈枠組みで、感情面・直近の心配事に関わるテーマとして割り当てた位置です。',
    '左目の下': '本アプリ独自の解釈枠組みで、身内との関係に関わるテーマとして割り当てた位置です。',
    '額': '本アプリ独自の解釈枠組みで、思考・プレッシャー・目上との関係に関わるテーマとして割り当てた位置です。',
    'ほほ': '本アプリ独自の解釈枠組みで、対人関係・社会的な立場に関わるテーマとして割り当てた位置です。',
    '口の横': '本アプリ独自の解釈枠組みで、言葉・発言・本音に関わるテーマとして割り当てた位置です。',
    '顎': '本アプリ独自の解釈枠組みで、晩年・家庭・生活基盤に関わるテーマとして割り当てた位置です。',
  };
  ...
  // 検出数についての注記
  //
  // 2026-08版で見直し: 以前は「検出数が多い=内面が複雑・強いストレス下」
  // という、件数の多さをネガティブな評価に結びつける文言だったが、
  // 「検出数が多いことと、複雑さ・不安定さの評価は別」という指摘を
  // 受け、件数そのものへの評価は行わず、各項目が独立して意味を持つ旨を
  // 述べるだけの文言に変更した。
  // -----------------------------------------------------------------
  function countComplexity(count) {
    if (count <= 1) return '検出された願相は1つです。';
    return '検出された願相は' + count + '個です。件数の多さ自体を複雑さや不安定さの表れとは捉えず、1つ1つを独立した意味を持つものとして読み解きます。';
  }
  ...
  function buildGansouReport(input) {
    ...
    var positionMeaning = POSITION_MEANINGS[position] || 'この位置に対応する意味づけは登録されていません。';
    ...
    var message = '「' + position + '」に検出された願相は、向き(' + orientation + ')から見て' +
      ...
      'を示します。' + typeMeaning + complexityNote +
      'これは文献に基づく鑑定ではなく、本アプリが独自に設定した娯楽目的の解釈であり、実際の心理状態や人間関係を断定するものではありません。';
  ...
  // 十二宮(命宮・兄弟宮・夫妻宮・子女宮・財帛宮・疾厄宮・遷移宮・
  // 奴僕宮・官禄宮・田宅宮・福徳宮・父母宮)という「枠組みの名前」自体は
  // 伝統的な人相学の用語だが、以下の各宮の説明文は本アプリが一般的な
  // 通説を要約して独自に書いたものであり、`data/`の出典付きデータとは
  // 別物(このファイル冒頭の出典注記を参照)。
  var PALACE_MEANINGS = {
  ...
  function buildAbstractGansouReport(input) {
    ...
    if (resolvedPalaceName && PALACE_MEANINGS[resolvedPalaceName]) {
      ...
      palaceMeaning = 'この位置に対応する宮の分類は特定できませんでした。';
    }
    ...
    var message = '「' + locationLabel + '」' + (resolvedPalaceName ? '(' + resolvedPalaceName + ')' : '') +
      'に検出された抽象的な隠れ相は、向き(' + directionJa + ')から見て' + directionThemeText + 'を示します。' +
      clarityNote +
      'これは文献に基づく鑑定ではなく、本アプリが独自に設定した娯楽目的の解釈であり、実際の心理状態や人間関係を断定するものではありません。';
    if (message.length > 220) message = message.slice(0, 217) + '…';
